...
const SiteSettings = ({ settings, setSettings }) => {
  ...
  const handleFileChange = useCallback((e, type) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setSettings(s => ({ ...s, [type]: reader.result }));
      };
        ...
        <div className="flex items-center space-x-4">
          <div>
            ...
            <div className="mt-2 w-24 h-24 border-2 border-dashed rounded-lg flex items-center justify-center bg-muted">
              {settings.logo ? <img src={settings.logo} alt="Logo Preview" className="h-full w-full object-contain" /> : <Icons.Image className="h-8 w-8 text-muted-foreground" />}
            </div>
            <Button variant="outline" size="sm" className="mt-2" onClick={() => logoInputRef.current.click()}>{t('uploadLogo')}</Button>
          ...
          <div>
            ...
            <div className="mt-2 w-16 h-16 border-2 border-dashed rounded-lg flex items-center justify-center bg-muted">
              {settings.favicon ? <img src={settings.favicon} alt="Favicon Preview" className="h-full w-full object-contain" /> : <Icons.Image className="h-6 w-6 text-muted-foreground" />}
            </div>
            <Button variant="outline" size="sm" className="mt-2" onClick={() => faviconInputRef.current.click()}>{t('uploadFavicon')}</Button>
