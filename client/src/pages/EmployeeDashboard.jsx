...
import { Calendar, DollarSign, TrendingUp, RefreshCw, Target, Search } from "lucide-react";
...
const MOCK_RESERVATIONS = [
    { id: 1, date: "2025-08-01", bookingType: "Online", status: "Completed", customer: "John Doe", total: 420, cost: 220, refunded: 0 },
    { id: 2, date: "2025-08-03", bookingType: "Phone", status: "Pending", customer: "Jane Smith", total: 180, cost: 60, refunded: 0 },
    { id: 3, date: "2025-08-05", bookingType: "Walk-in", status: "Cancelled", customer: "Omar Khaled", total: 0, cost: 0, refunded: 0 },
    { id: 4, date: "2025-08-10", bookingType: "Online", status: "Completed", customer: "Sara Ali", total: 690, cost: 300, refunded: 50 },
    { id: 5, date: "2025-08-14", bookingType: "Phone", status: "Completed", customer: "Bob Brown", total: 240, cost: 120, refunded: 0 },
];
...
export default function EmployeeDashboard() {
    ...
    const filtered = useMemo(() => {
        ...
        return MOCK_RESERVATIONS.filter((r) => {
            ...
            const matchType = bookingType === "All" || r.bookingType === bookingType;
    ...
    }, [from, to, bookingType, status, customer]);
        ...
        <div className="min-h-screen p-2 md:p-3">
            <div className="max-w-7xl mx-auto space-y-4">
                        ...
                        <Select.Root value={bookingType} onValueChange={setBookingType}>
                            ...
                            <Select.Content className="bg-black rounded-lg shadow-lg border border-gray-200 mt-1">
                                ...
                                <Select.Viewport className="p-1">
                                    {["All", "Online", "Phone", "Walk-in"].map((opt) => (
                                        <Select.Item
                                            key={opt}
                                            value={opt}
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <KpiCard className="bg-black" icon={<DollarSign className="text-blue-600" />} label="Total Sales" value={currency(kpis.totalSales)} />
                        ...
                        <tbody>
                            {filtered.map((r) => (
                                <tr key={r.id} className="border-b last:border-none">
                                    ...
                                    <td className="py-2">{r.bookingType}</td>
                                    <td className="py-2">
                                        <span className={`px-2 py-1 rounded-lg text-xs border ${STATUS_STYLES[r.status]}`}>{r.status}</span>
                                    </td>
                                    <td className="py-2">{r.customer}</td>
                                    <td className="py-2">{currency(r.total)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <Dialog.Root open={editingTarget} onOpenChange={setEditingTarget}>
                ...
                <Dialog.Content className="fixed top-1/2 left-1/2 w-[90%] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-xl">
                    ...
                    <div className="mt-4 flex justify-end gap-2">
                        ...
                        <button onClick={() => { setMonthlyTarget(Number(targetInput)); setEditingTarget(false); }} className="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">
                            Save
                        </button>
                    </div>
                </Dialog.Content>
