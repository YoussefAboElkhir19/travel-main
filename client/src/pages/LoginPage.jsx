...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://travel-server.test/api/login", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          email: credentials.identifier, // Laravel expects 'email'
      ...
      if (response.ok && result.user && result.token) {
        toast({ title: "Login Successful", description: `Welcome back, ${result.user.user_name}!` });

        // تحديث الـ context + حفظ البيانات
        login(result.user, result.token);

        // التحويل لصفحة الحضور
